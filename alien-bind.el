...
(defconst +python-left-stripper+ (format "%s\" + " +left-delim+))
(defconst +python-right-stripper+ (format " + \"%s" +right-delim+))

(defun python-escape-quotes (str)
  (replace-regexp-in-string "\"" "\\\\\"" str))

(defun python-strip-vars (str)
  (replace-regexp-in-string 
   +left-delim+ +python-left-stripper+
   (replace-regexp-in-string +right-delim+ +python-right-stripper+
							 str)))

(defun impl-python-inject-lisp (form code)
  (format 
"%s
print \"%s\"" code
		  (python-strip-vars 
		   (python-escape-quotes (format "%S" form)))))

(defun impl-python-assemble-cmdline (file)
  (concat "python \"" file "\""))
